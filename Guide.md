...
- Windows 10 Telemetry, Cortana and More (more privacy oriented)
   ...
   >> This should have a tick: Privacy, Cortana, Remove all built-in Apps except Defaults (if you want a clean windows though just tick
         Bloatware), On "App permissions" tick everything EXCEPT microphone and camera (this ensures that you will be able to use
         these in certain programs) -> tick:Gaming, Security.
   >> click on "Apply selected" and check with "Analyze" if the changes have successfully been made
...
This topic is about a debloating script i forked and worked on as best as i can, i will try to explain exactly what this script will be doing. I made two different scripts depending if you want to get the option to install programms and the debloating or just run the debloating.
  ...
  This will do the following steps/functions:
    ...
    - Set Windows updates to "only security updates" (Don't worry if it says on "updates" that "Some options are managed through your company" are shown on the windows update page, this is because the script disables quality updates to only allow security ones)
	- Fixes problems that other scripts causes (lock screen and personalization options restricted)
		...
		>> Right Click Windows Menu -> Select PowerShell Prompt (Admin) OR Select Command Prompt (Admin)
		>> Now you can decide between a script running the install prompts or a script running without any.
		>> type in or paste for no installs: "iex(New-Object Net.WebClient).DownloadString('https://git.io/Jkc16')"
		>> type in or paste for installs: "iex((New-Object System.Net.WebClient).DownloadString('https://git.io/JkZwQ'))"
		>> now it will run the scripts and do the stuff mentioned above.
...
- CPU and Memory Performance
   ...
   >> Check for the CPU Performance Info
   >> Look for the "Enabled cores" and "Parked cores", your "Enabled cores" should show the same amount as "Logical processors"
         while it should show 0 for "Parked cores"
   >> All index tabs at the bottom should be set to 100% after you have done that hit 'Apply'


- Turn off >Mouse Acceleration<

  >> Press your "Windows-Key"
  >> Go into settings and search for mouse and click on "mouse settings"
  >> There will be something called "Additional mouse options" on the right hand side, click on it.
  >> Then you go to the Tab "Pointer Options" and take off the tick for "Enhanced pointer precision"

- Stop unneeded background Services

  >> Press the "Windows-Key" + "R" and type in "msconfig"
  >> Go to "Services" and tick the box that says "exclude all Microsoft services"
  >> now look for any services that you don't need and untick them (remember to not disable any Anti-Cheat or AntiVirus Services)
...
- How to set up your Firefox Browser
   ...
   >> Download "profile.zip" from either the discord server or DM me
   >> Press "Windows Key" + "R" on the keyboard. A Run dialog will open
   >> In the Run dialog box, type in:firefox.exe -P (You can use -P, -p or -ProfileManager (any of them should work))
   >> Click OK. The Firefox Profile Manager window should be open now.
   >> Click on "create Profile" name the profile 'Main' and then "start Firefox"
   >> Now type about:profiles and search for the Main profile, open the Root-folder
   >> Delete everything in that folder and paste all files from profile.zip in there -> you can close the folder
   >> Now go back to your Firefox about:config window and press "F5", search for your profile named Main
   >> Click on "Set as default profile" and restart your Firefox
		>> On the right hand side click on the menu button -> Options -> Files and Applications (its in General when you scroll down)
		>> Check if its set to "Ask every time where to save the file to" if not select it.
		>> Go to "Home" -> "Homepage and New Tab" change it to User defined address
		>> Paste in this: https://duckduckgo.com/
		>> Go to "Search" and change the Default Searchengine to DuckDuckGo instead of Google!!
		>> Scroll down and remove every search engine except DuckDuckGo and Startpage
		>> Click the menu icon on the top right -> "Add-Ons" -> click on the cogwheel symbol -> "Check for Updates"
		>> Follow the "uBlock Origin Settings" topic in this guide
	>> Your Firefox is now ready to go!


- How to set up your Brave Browser

  >> First of all DO NOT skip the welcome tour!!
   >> You don't have to import bookmarks and settings, if you have freshly installed Firefox Browser with this Guide you can import from Mozilla Firefox otherwise just move on!
   >> On "Set default search engine" you can decide between "DuckDuckGo" or "Qwant"
   >> DONT enable "Brave Rewards" !!
   >> Go to the Menu (top right corner) -> "Settings" -> "On Startup" topic select "Open a specific page or set of pages" -> "Add a new page"
   >> Depending on what search engine you chose at the tour type in "DuckDuckGo.com" for DuckDuckGo or "Qwant.com" for Qwant
   >> "Appearance" -> you want this checked: "Hide Brave Rewards button", "Show home button" (Enter custom address: "DuckDuckGo.com" for DuckDuckGo or "Qwant.com" for Qwant) and "Show bookmarks"
   >> "New Tab Page" -> uncheck "Brave Rewards"
   >> "Shields" -> "Default View" set it to "Advanced View"
   >> "Extensions" -> "Web3 provider for using Dapps" set it to "None"
   >> Disable "Hangouts"
   >> "Additional settings" -> "Privacy and security" -> check "Send a "Do Not Track" request with your browsing traffic" - uncheck "Allow sites to check if you have payment methods saved"
   >> Your Brave is now ready to go! 	(Check out "Recommended Add-Ons" in this Guide too! Brave Browser doesn't need uBlock Origin! Also it is not compatible with Mozilla addons but the Chrome Store)


- How to set up your Un-Googled Chromium

   >> [Download the latest release here](https://ungoogled-software.github.io/ungoogled-chromium-binaries/releases/)
   >> Go to menu in the top right corner -> "Settings" -> "Appearance" -> "Search engine" -> "Search engine used in the address bar" set to "DuckDuckGo"
   >> "Advanced" -> "System" -> uncheck "Use hardware acceleration when available"
   >> go to chrome://flags/#extension-mime-request-handling in your address bar -> set the highlighted option to "Always prompt for install" -> "Relaunch"
   >> Now download the following Add-On:
     >> https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&prodversion=86.0.4240.183&x=id%3Dcjpalhdlnbpafiamejdnhcphjbkeiagm%26installsource%3Dondemand%26uc
     >> https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&prodversion=86.0.4240.183&x=id%3Dgcbommkclmclpchllfjekcdonpmejbdp%26installsource%3Dondemand%26uc
     >> https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&prodversion=86.0.4240.183&x=id%3Dlckanjgmijmafbedllaakclkaicjfmnk%26installsource%3Dondemand%26uc
     >> https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&prodversion=86.0.4240.183&x=id%3Djchobbjgibcahbheicfocecmhocglkco%26installsource%3Dondemand%26uc
     >> https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&prodversion=86.0.4240.183&x=id%3Dpkehgijcmpdhfbdbbnkijodmdjhbjlgp%26installsource%3Dondemand%26uc
     >> [Chromium Web Store](https://github.com/NeverDecaf/chromium-web-store/releases/download/v1.1.1/Chromium.Web.Store.crx)
   >> Now on the right corner there should be a puzzle icon, click it -> click on the icon ![Icon](https://www.imagebanana.com/s/big/1942/puvEBBW5.png) and update every extension
   >> Follow the "uBlock Origin Settings" topic in this guide.
   >> Your Un-Googled Chromium is now ready to go!
...
>> On the top right side of the browser should be a red shield icon ![Icon](https://www.imagebanana.com/s/big/1942/8VhS0ifp.png), click on that -> click on this ![icon](https://www.imagebanana.com/s/big/1942/SXpCY32V.png)
		>> Checkmark all the options "Prevent WebRTC from leaking local IP addresses" and "Block CSP reports" on the "Privacy" column
		>> Click on "Filter lists" in the upper corner -> click on "[Number] network filters ＋ [Number] cosmetic filters from:" ![Topic](https://www.imagebanana.com/s/big/1942/9vMapX50.png)
		>> Checkmark everything as follows [Link](https://www.imagebanana.com/s/big/1942/KOv5KRaI.png) and [Link](https://www.imagebanana.com/s/big/1942/sVtMXuRp.png)
		>> "Apply changes" -> "Update now"
			>> Follow to this [link](https://jspenguin2017.github.io/uBlockProtector/#extra-installation-steps-for-ublock-origin) and follow the "Extra installation steps for uBlock Origin" instructions, to "Subscribe" simply click on the Hyperlink and press OK on the Popup Window.
...
This will help you a bit with your pc performance if you have older hardware mostly this is no longer a huge difference
   ...
   >> Open "Windows Search" (right click the windows menu -> "Search")
   >> Type in "Performance" -> click the first result
   >> Now in visual effects you can turn it to "Adjust for best performance"
   >> click "Apply"
...
This will remove the "recently opened files" section when you open the file explorer
   ...
   >> Press "Windows key" + "I" -> search for "file options" -> click on "File Explorer Options"
   >> Change "Open File Explorer to" to "This PC"
   >> At the bottom in the "Privacy" topic untick "Show frequently used folders in quick access
   >> Click "Apply"
...
[Instruction Video](https://www.youtube.com/watch?v=uJ9fyOkJFfc)
   >> Put the video on fullscreen
   >> Try to focus on the black crosshair/cross in the middle
   >> If you see a green dot moving in a circle around it you are doing everything correctly otherwise try around, you need to fully focus on the center.
